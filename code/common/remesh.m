...
function [TRIV_, X_, Y_, Z_] = remesh(surface, varargin)
...
verts = length(X); %min(length(X), 2000);
faces = size(TRI,1); %min(size(TRI,1), 5000);
verbose = 1;
...
if nargin > 1,
    ...
    if isfield(options, 'contract'),
        if strcmpi(options.contract,'face') || strcmpi(options.contract,'faces'),
            opt(4) = 1;
        elseif strcmpi(options.contract,'edge') || strcmpi(options.contract,'edges'),
            opt(4) = 0;
        else
